% dataprocessing_xichu_012_jie.m
...
%% ========== MED 解卷积参数 ==========
USE_MED         = true;          % 是否启用 MED 解卷积
MED_FILTER_SIZE = 40;            % 滤波器长度 (30~60 适合512点信号)
MED_MAX_ITER    = 30;            % 最大迭代次数
MED_TOL         = 1e-6;          % 收敛容差
...
output_root = 'C:\Users\ClearNight\Desktop\PINN\CWRU_CrossLoad_jie';
...
%% ================================================================
% 第1步: 三个负载各自生成 MED+CWT 矩阵 (保存到临时目录)
%% ================================================================
fprintf('========== 第1步: 逐负载生成 MED + CWT (解卷积版本) ==========\n');
...
function y = med_deconv(x, L, max_iter, tol)
% Minimum Entropy Deconvolution (MED)
% 通过最大化输出信号的峰度(kurtosis)来增强故障冲击特征
%
% 原理: 传感器信号 x = s * h + n
%       其中 s=故障冲击, h=传递路径脉冲响应, n=噪声
%       MED 估计一个逆滤波器 f, 使得 y = x * f ≈ s
%       优化目标: max kurtosis(y) = E[y^4] / E[y^2]^2
%
% 参考: Wiggins (1978), Endo & Randall (2007)
    ...
    % 镜像填充边界，保证输出与输入等长
    pad = floor(L / 2);
    x_pad = [flipud(x(1:pad)); x; flipud(x(end-pad+1:end))];

    % 构建卷积矩阵 X: 每一行是滤波器覆盖的信号段
    M = N;
    X = zeros(M, L);
    for i = 1:M
        start = i;
    ...
    for iter = 1:max_iter
        ...
        lambda = 0.01 * trace(Rxx) / L;
        f_new = (Rxx + lambda * eye(L)) \ rxb;

        f_norm = norm(f_new);
        if f_norm < eps, break; end
        f_new = f_new / f_norm;

        if norm(f_new - f) / f_norm < tol
            f = f_new;
            break;
        end
